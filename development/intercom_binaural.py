...
class Intercom_binaural(Intercom_bitplanes):
  ...
  def record_and_send(self, indata):
    ...
    sys.stderr.write("\nMAX: {} LIMITE: {}".format(valmax, limite)); sys.stderr.flush()

    #Minimum 2 bitplane is always sent (the rest depending on the congestion)
    #Sending the first bitplane (BIT 15)
    for bitplane_number in range(self.number_of_channels*16-1, (self.number_of_channels*16)-(self.number_of_channels), -1):
    ...
    for bitplane_number in range(self.number_of_channels*limite, (self.number_of_channels*limite)-(self.number_of_channels), -1):
      bitplane = (package[:, bitplane_number%self.number_of_channels] >> bitplane_number//self.number_of_channels) & 1
      bitplane = bitplane.astype(np.uint8)
      bitplane = np.packbits(bitplane)
  ...
  def record_send_and_play_stereo(self, indata, outdata, frames, time, status):
    ...
    self.missed_current=self._buffer_control[(self.played_chunk_number) % self.cells_in_buffer][0]

    ESP=self._buffer_control[(self.played_chunk_number) % self.cells_in_buffer][1]
    ACT=self._buffer_control[(self.played_chunk_number) % self.cells_in_buffer][0]
    #sys.stderr.write("\nESP: {} ACT: {} CURR: {}".format(ESP,ACT,self.missed_current)); sys.stderr.flush()
    
    #Calculation to reset congestion
    self.missed_packets_own = int(self.missed_packets_own*0.8 + self.missed_current*0.2)-10
